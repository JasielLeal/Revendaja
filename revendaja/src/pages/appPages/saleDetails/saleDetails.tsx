            ...
            <View className='px-5 pt-16'>
                <View className='flex flex-row justify-between'>
                    <TouchableOpacity onPress={() => navigation.goBack()}>
                        <Icon name='chevron-back' size={20} color={colorScheme === "dark" ? "#fff" : ""} />
                    </TouchableOpacity>
                    ...
                    Platform.OS === 'ios' ?
                        <>
                            <View className='flex flex-col justify-center items-center mt-7'>
                                <Text className="dark:bg-[#303030] bg-forengroundLight p-5 rounded-full">
                                    <Icon name='checkmark' size={20} color={"#FF7100"} />
                                </Text>
                                <Text className='dark:text-white font-medium mt-5 text-xl'>R$ {formatCurrency(String(sale.totalPrice))}</Text>
                            ...
                            <View className='mt-7'>
                                <Text className='dark:text-white text-black font-medium'>Sobre a compra</Text>
                            </View>

                            <View className='bg-forenground p-3 rounded-xl mt-5'>
                        ...
                        <>
                            <View className='flex flex-col justify-center items-center mt-7'>
                                <Text className="dark:bg-forenground bg-forengroundLight p-5 rounded-full">
                                    <Icon name='checkmark' size={20} color={"#FF7100"} />
                                </Text>
                                <Text className='dark:text-white font-medium mt-5'>R$ {formatCurrency(sale.totalPrice)}</Text>
                            ...
                            <View className='dark:bg-forenground bg-forengroundLight p-3 rounded-xl mt-5'>
                                <View className='flex flex-row items-center justify-between'>
                            ...
                            <View>
                                {sale.saleItems.map((product: any) => (
                                    <View
                                        key={product.id}
                                        className='flex flex-row items-center justify-between dark:bg-forenground bg-forengroundLight p-3 rounded-xl mb-2'
                                    >
                                        <View className='flex flex-row items-center'>

                                            <Text className='dark:text-white mr-3 text-xs'>{product.quantity}x</Text>
                                            <Text className='dark:text-white w-[140px] text-xs' numberOfLines={1} ellipsizeMode="tail">
                                                {product.stock.customProduct?.name || product.stock.product?.name}
                                            </Text>
                                        </View>
                                        <Text className='dark:text-white text-xs'>R$ {formatCurrency(String(product?.price))}</Text>
                                    </View>
                                ))}
                            </View>
                        </>
